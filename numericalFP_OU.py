...
nev = 50
tol = 0. #relative accuracy for eigenvalues
...
# Define drift
def drift(x):
    """Input is a vector, output as well"""
    return np.dot(A, x)
...
for d in np.arange(dim):
    ...
    dx[d] = x[d][1] - x[d][0]
N = np.prod(nx) #product of grid points in every direction
